#!/usr/bin/env bash
# Play an FCEUX movie (.fm2) against a ROM and screenshot at frame N.
#   scripts/emu_movie_shot.sh <rom> <movie.fm2> <out.png> [frames]
source "$(dirname "$0")/lib.sh"
ROM="${1:?rom}"; MOVIE="${2:?movie}"; OUT="${3:?out.png}"; FRAMES="${4:-800}"
need fceux; [ -f "$ROM" ] || die "ROM not found: $ROM"; ROM="$(cd "$(dirname "$ROM")" && pwd)/$(basename "$ROM")"; MOVIE="$(cd "$(dirname "$MOVIE")" && pwd)/$(basename "$MOVIE")"; OUT="$(cd "$(dirname "$OUT")" && pwd)/$(basename "$OUT")"
rm -f "$OUT"; export NESSHOT_OUT="$OUT" NESSHOT_FRAMES="$FRAMES" NESSHOT_PRESS=""
...
start_display
FCEUX_CMD="fceux --playmov "$MOVIE" --loadlua "$ROOT/scripts/emu_screenshot.lua" "$ROM""
...
PID=$!
for i in $(seq 1 120); do kill -0 $PID 2>/dev/null || break; [ -f "$OUT" ] && { sleep 1; break; }; sleep 1; done; stop_emu $PID
[ -f "$OUT" ] && log "screenshot: $OUT" || die "no screenshot produced"
