#!/usr/bin/env bash
# Dump palette + nametable contents after N frames.  scripts/emu_dump.sh <rom> [out.txt] [frames]
source "$(dirname "$0")/lib.sh"
ROM="${1:?usage: emu_dump.sh <rom> [out] [frames]}"; OUT="${2:-${ROM%.nes}.dump.txt}"; FRAMES="${3:-240}"
...
start_display
FCEUX_CMD="fceux --loadlua "$ROOT/scripts/emu_dump.lua" "$ROM""
...
[ -f "$OUT" ] && cat "$OUT" || die "no dump produced"
