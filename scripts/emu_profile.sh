#!/usr/bin/env bash
# Per-frame CPU profile by routine.  scripts/emu_profile.sh <rom> <ld65 map> [from=60] [to=120] [button] [press_at] [top=16]
HERE="$(cd "$(dirname "$0")" && pwd)"
ROM="${1:?rom}"; MAP="${2:?map}"; FROM="${3:-60}"; TO="${4:-120}"; PRESS="${5:-}"; AT="${6:-0}"; TOP="${7:-16}"
[ -f "$ROM" ] || { echo "ROM not found: $ROM" >&2; exit 1; }; [ -f "$MAP" ] || { echo "map not found: $MAP (build with ld65 -m)" >&2; exit 1; }
ROM="$(cd "$(dirname "$ROM")" && pwd)/$(basename "$ROM")"; MAP="$(cd "$(dirname "$MAP")" && pwd)/$(basename "$MAP")"
OUT="$(mktemp "${TMPDIR:-/tmp}/nesprof.XXXXXX").txt"; rm -f "$OUT"
export NESPROF_OUT="$OUT" NESPROF_MAP="$MAP" NESPROF_FROM="$FROM" NESPROF_TO="$TO" NESPROF_PRESS="$PRESS" NESPROF_AT="$AT" NESPROF_TOP="$TOP"
FCEUX="fceux"
...
start_display
bash -c "$FCEUX --loadlua '$HERE/emu_profile.lua' '$ROM'; true" >/dev/null 2>&1 &
PID=$!
for i in $(seq 1 240); do kill -0 $PID 2>/dev/null || break; [ -s "$OUT" ] && { sleep 1; break; }; sleep 1; done; stop_emu $PID
[ -s "$OUT" ] && cat "$OUT" || { echo "no profile produced" >&2; exit 1; }
rm -f "$OUT"
