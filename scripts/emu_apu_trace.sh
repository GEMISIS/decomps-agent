#!/usr/bin/env bash
# APU register write trace.  scripts/emu_apu_trace.sh <rom> <out.txt> [from=0] [to=300] [button] [press_at]
HERE="$(cd "$(dirname "$0")" && pwd)"
need() { command -v "$1" >/dev/null 2>&1 || { echo "missing $1" >&2; exit 1; }; }
die() { echo "error: $*" >&2; exit 1; }
log() { echo "$*" >&2; }
ROM="${1:?rom}"; OUT="${2:?out}"; FROM="${3:-0}"; TO="${4:-300}"; PRESS="${5:-}"; PRESS_AT="${6:-0}"
need fceux; [ -f "$ROM" ] || die "ROM not found: $ROM"; ROM="$(cd "$(dirname "$ROM")" && pwd)/$(basename "$ROM")"; OUT="$(cd "$(dirname "$OUT")" && pwd)/$(basename "$OUT")"
rm -f "$OUT" "$OUT.part"; export NESAPU_OUT="$OUT" NESAPU_FROM="$FROM" NESAPU_TO="$TO" NESAPU_PRESS="$PRESS" NESAPU_PRESS_AT="$PRESS_AT"
FCEUX="fceux"
...
start_display
FCEUX_CMD="$FCEUX --loadlua \"$HERE/emu_apu_trace.lua\" \"$ROM\""
...
[ -f "$OUT" ] || die "no trace produced"
# summary + capped listing on stdout (tool-friendly)
echo "$(wc -l < "$OUT" | tr -d ' ') APU register writes in frames $FROM-$TO; writes per frame (frame count):"
cut -d' ' -f1 "$OUT" | uniq -c | awk '{printf "%s:%s ", $2, $1}' | head -c 600; echo
head -n "${NESAPU_MAX_LINES:-200}" "$OUT"
