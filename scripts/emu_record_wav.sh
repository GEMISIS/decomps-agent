#!/usr/bin/env bash
# Record audio to WAV.  scripts/emu_record_wav.sh <rom> <out.wav> [frames=600] [button] [press_at]
source "$(dirname "$0")/lib.sh"
ROM="${1:?rom}"; OUT="${2:?out.wav}"; FRAMES="${3:-600}"; PRESS="${4:-}"; PRESS_AT="${5:-120}"
need fceux; [ -f "$ROM" ] || die "ROM not found: $ROM"; ROM="$(cd "$(dirname "$ROM")" && pwd)/$(basename "$ROM")"; OUT="$(cd "$(dirname "$OUT")" && pwd)/$(basename "$OUT")"
rm -f "$OUT"; TMPPNG="$(mktemp "${TMPDIR:-/tmp}/nesshot.XXXXXX").png"; export NESSHOT_OUT="$TMPPNG" NESSHOT_FRAMES="$FRAMES" NESSHOT_PRESS="$PRESS" NESSHOT_PRESS_AT="$PRESS_AT"
...
start_display
FCEUX_CMD="fceux --sound 1 --soundrate 44100 --soundq 1 --soundrecord \"$OUT\" --loadlua \"$ROOT/scripts/emu_screenshot.lua\" \"$ROM\""
...
PID=$!
for i in $(seq 1 180); do kill -0 $PID 2>/dev/null || break; [ -f "$TMPPNG" ] && { sleep 1; break; }; sleep 1; done; stop_emu $PID
rm -f "$TMPPNG"; [ -s "$OUT" ] && log "recorded $(stat -f %z "$OUT") bytes → $OUT" || die "no WAV produced"
